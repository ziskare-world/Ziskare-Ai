...
class TaskAgent(BaseAgent):
    ...
    def __init__(self, ai: Optional[Any] = None, max_steps: int = 5, silent: bool = False):
        super().__init__(
            ...
            temperature=0.1,  # Low temperature for stable tool syntax
            silent=silent
        )
    ...
    def _parse_action(self, text: str) -> Optional[Tuple[str, Dict[str, Any]]]:
        ...
        Supported format: Action: tool_name({"arg": "val"}) or Action: tool_name(arg="val")
        """
        match = re.search(r'Action:\s*([a-zA-Z_]+)\s*\((.*)\)', text, re.DOTALL)
        if not match:
            # Fallback: check JSON block
        ...
        # Try regex parsing for key="value" or key='value' or single argument
        args = {}
        if "=" in raw_args:
    ...
    def execute_task(self, task: str, verbose: bool = True) -> Dict[str, Any]:
        ...
        current_prompt = f"User Task: {task}\nBegin!"

        for step in range(1, self.max_steps + 1):
            # Prompt the model with current execution trace
            context = "\n\n".join(history)
